...
void read_experiment_parameters(std::string conf_filename, uint32_t exp_type) {
    ...
    /* pim parameter */
    params.util_file = "";
    params.drop_reorder_file = "";
    params.queuing_file = "";
    params.queuing_file_2 = "";
    params.sw_queuing_file = "";
    params.param_b = 12;
    params.l_mult = 1;
    ...
    params.sns_min_rate = MAXFLOAT; //for sns
    params.num_data_packets_per_sim_ack = 1; // Default: 1 data packets per SIM ACK
    params.worst_rrt_factor = 1.0;
    while (std::getline(input, line)) {
        ...
        if (line[0] == '#' || (line.length() >= 2 && line.substr(0, 2) == "//")) {
            continue;
        }
        ...
        else if (key == "queueing_file") {
            continue;
            lineStream >> params.queuing_file;
        }
        else if (key == "queueing_file_2") {
            lineStream >> params.queuing_file_2;
        }
        else if (key == "sw_queueing_file") {
            continue;
            lineStream >> params.sw_queuing_file;
        }
        else if (key == "util_all_file") {
            lineStream >> params.util_all_file;
        }
        else if (key == "fct_file") {
            lineStream >> params.fct_file;
        }
        else if (key == "drop_pkt_file") {
            params.total_drop_sim_pkts = 0;
            params.total_drop_data_pkts = 0;
            params.total_sim_pkts = 0;
            params.total_data_pkts = 0;
            lineStream >> params.drop_pkt_file;
        }
        else if (key == "num_hosts") {
            lineStream >> params.num_hosts; 
        }
        else if (key == "clock_drift") {
            lineStream >> params.clock_drift; 
        }
        else if (key == "worst_rrt_factor") {
            lineStream >> params.worst_rrt_factor;
        }
        ...
        else if (key == "debug_sns") {
            lineStream >> params.debug_sns;
        }
        else if (key == "debug_sns2") {
            lineStream >> params.debug_sns2;
        }
        else if (key == "num_data_packets_per_sim_ack") {
            lineStream >> params.num_data_packets_per_sim_ack;
        }
        else if (key == "big_mtu") {
            lineStream >> params.big_mtu;
        }
        else if (key == "core_div_2") {
            lineStream >> params.core_div_2;
        }
        else if (key == "param_b") {
            lineStream >> params.param_b;
        }
        else if (key == "l_mult") {
            lineStream >> params.l_mult;
        }
        else if (key == "synced_mode") {
            lineStream >> params.synced_mode;
        }
        ...
        params.fastpass_epoch_time = (params.mss + params.hdr_size) * 8 * (params.fastpass_epoch_pkts + 0.5) / params.bandwidth;
        // params.ruf_epoch_time = (params.mss + params.hdr_size) * 8 * (RUF_EPOCH_PKTS + 0.5) / params.bandwidth;
    ...
    // assert(false);
    if (params.big_mtu == 1) {
        params.mss = 4000 - params.hdr_size;
        params.sns_simulation_frq_size = 100;
        params.sns_simulation_frq_size = 101;
        params.sns_simulation_frq_size = (4000 + params.l_mult*40) / 40;
    } else if (params.big_mtu == 0) {
        params.mss = 1460;
        params.sns_simulation_frq_size = 37.5;
        params.sns_simulation_frq_size = 38.5;
        params.sns_simulation_frq_size = (1460 + params.l_mult*40) / 40;
    } else if (params.big_mtu == 2) {
        params.mss = 9000 - params.hdr_size; // 9000 is the jumbo frame size also change the SNS_SIMULATION_FRAQ_SIZE
        params.sns_simulation_frq_size = 225;
        params.sns_simulation_frq_size = 226;
        params.sns_simulation_frq_size = (9000 + params.l_mult*40) / 40;
    } else {
        assert(false);
    }
    // params.mss = 1460;
