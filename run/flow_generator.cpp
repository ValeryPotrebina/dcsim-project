...
void PoissonLocalFlowGenerator::make_flows() {
    ...
    double lambda_local = params.bandwidth * params.local_load / (params.mean_flow_size * 8.0 / params.mss * (params.mss + params.hdr_size));
    double lambda_non_local = params.bandwidth * params.os_ratio * params.remote_load / (params.mean_flow_size * 8.0 / params.mss * (params.mss + params.hdr_size));
    ...
    //* [expr ($link_rate*$load*1000000000)/($meanFlowSize*8.0/params.mss*(params.mss + params.hdr_size))]
...
void PoissonFlowGenerator::make_flows() {
    ...
    params.mean_flow_size = nv_bytes->mean_flow_size;
    double lambda = params.bandwidth * params.load / (params.mean_flow_size * 8.0 / params.mss * (params.mss + params.hdr_size));
    double lambda_per_host = lambda / (topo->hosts.size() - 1);
    // std::cout << "Lambda: " << lambda << std::endl;
    // std::cout << "lambda_per_host: " << lambda_per_host << std::endl;
    // std::cout << "mean_flow_size: " << params.mean_flow_size << std::endl;
    ...
    // nv_intarr = new ExponentialRandomVariable(0.0000000000000001);
    // std::cout << "nv_intarr->value(): " << nv_intarr->value() << std::endl;
    ...
    for (uint32_t i = 0; i < topo->hosts.size(); i++) {
        for (uint32_t j = 0; j < topo->hosts.size(); j++) {
            if (i != j) {
                ...
                //std::cout << "first_flow_time: " << (first_flow_time-1.0)*10e6 << std::endl;
                add_to_event_queue(
                    new FlowCreationForInitializationEvent(
                        first_flow_time,
                        topo->hosts[i], 
...
void FlowReader::make_flows() {
    ...
    while (std::getline(input, line)) {
        ...
        double start_time, temp2, collective_id, temp1_double;
        uint32_t size, s, d;
        uint32_t id;
        
        // <id> <start_time> <collective_id> <temp> <size in packets> <temp> <temp> <src> <dst>

        if (!(iss >> id >> start_time >> collective_id >> temp2 >> size >> temp1_double >> temp2 >> s >> d)) {
            break;
        }
        
        if (s >= topo->hosts.size() || d >= topo->hosts.size()) {
            std::cout << "ERROR: Host index out of bounds. s=" << s << " d=" << d << " topo size=" << topo->hosts.size() << std::endl;
            continue;
        }
        ...
        Flow* flow = Factory::get_flow(id, start_time, size, topo->hosts[s], topo->hosts[d], params.flow_type);
        if (collective_id - 1.1 > 0.001)
            flow->collective_id = int(collective_id);  // Set the collective ID
...
void IncastTM::make_flows() {
    ...
    // else
    //     nv_bytes = new CDFRandomVariable(filename);
    EmpiricalRandomVariable *nv_bytes = new ConstantVariable(int(data / this->incast / params.mss));
...
void WorstcaseTM::make_flows() {
    ...
    for (uint32_t i = 0; i < topo->hosts.size(); i++) {
        ...
        mean_flow_size = (7 * (i + 2) + 1) * params.mss;
        EmpiricalRandomVariable *nv_bytes = new ConstantVariable(mean_flow_size / params.mss);
        double lambda = params.bandwidth * params.load / (mean_flow_size * 8.0 / params.mss * (params.mss + params.hdr_size));
...
//    int matrix[3][3] =
//    {
//    {3*params.mss,    3*params.mss, 700*params.mss},
//    {3*params.mss,    0,      0},
//    {700*params.mss,  0,      0}
//    };
...
//    double lambda_per_pair = params.bandwidth * params.load / (flow_size * 8.0 / params.mss * (params.mss + params.hdr_size)) / num_sd_pair;
...
//     double lambda = params.bandwidth * params.load / (params.mean_flow_size * 8.0 / 1460 * (params.mss + params.hdr_size));
...
//     // [expr ($link_rate*$load*1000000000)/($meanFlowSize*8.0/1460*(params.mss + params.hdr_size))]
