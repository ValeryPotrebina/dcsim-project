...
class DCExpParams {
    public:
        ...
        std::string drop_reorder_file;        
        std::string queuing_file;
        std::string queuing_file_2;
        std::string sw_queuing_file;
        std::vector<std::vector<double>> specific_queueing_times;
        std::vector<std::vector<double>> specific_queueing_packets;

        std::string util_all_file;
        std::string fct_file;
        std::string drop_pkt_file;
        long total_drop_sim_pkts;
        long total_drop_data_pkts;
        long total_sim_pkts;
        long total_data_pkts;
        ...
        int pim_link_pkts;
        double get_full_pkt_tran_delay(uint32_t size_in_byte = 0)
        {
            if (size_in_byte == 0) size_in_byte = mss + hdr_size;
        ...
        bool debug_sns2;
        double sns_min_rate;
        uint32_t num_data_packets_per_sim_ack;  // Number of data packets to send per SIM ACK
        int big_mtu; // Use 4k MTU
        double sns_simulation_frq_size; // Frequency of SNS simulation packets
        bool core_div_2; // Divide core switch bandwidth by 2
        int simulation_queue_size; // Queue size for simulation packets at switches
        bool synced_mode;
        double param_b;
        int l_mult;
        double clock_drift;
        double worst_rrt_factor;
