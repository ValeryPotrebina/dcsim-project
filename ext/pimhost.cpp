...
void NewEpochEvent::process_event() {
    if (this->host == NULL) {
        std::cerr << "Error: NULL host in NewEpochEvent for epoch " << this->epoch << std::endl;
        return;
    }
       ...
       if(debug_host(this->host->id)) {
        ...
        if(this->host->epochs.count(this->epoch - 1) > 0){
            try {
                ...
                std::cerr << "Error erasing epoch " << (this->epoch - 1) << ": " << e.what() << std::endl;
            }
        }
        // if(this->host->token_send_evt != NULL && this->host->token_send_evt->is_timeout_evt) {
        //     this->host->token_send_evt->cancelled = true;
...
PimHost::PimHost(uint32_t id, double rate, uint32_t queue_type) : SchedulingHost(id, rate, queue_type) {
    ...
    this->epochs.clear(); // Make sure map is empty
}

void PimHost::start_new_epoch(double time, int epoch) {
    if (this == NULL) {
        std::cerr << "Fatal error: start_new_epoch called on NULL host for epoch " << epoch << std::endl;
        return;
    }
    
    if (this->epochs.count(epoch) > 0) {
        std::cerr << "Warning: Epoch " << epoch << " already exists for host " << this->id << std::endl;
        return;
    }
...
void PimHost::receive_token(PIMToken* pkt) {
    ...
    // if(!f->tokens.empty()) {
    //     t->timeout = fto->back().timeout + get_full_pkt_tran_delay(params.mss + params.hdr_size) - pkt->ttl;
    // }
...
void PIM_Vlink::send_token() {
    ...
    {
        this->schedule_token_proc_evt(params.get_full_pkt_tran_delay(params.mss + params.hdr_size/* + 40*/)
