...
SnsSimulationHostProcessingEvent::SnsSimulationHostProcessingEvent(double time, SnsHost *h) : Event(HOST_PROCESSING, time) {
    this->host = h;
    this->is_timeout = false;
}

SnsSimulationHostProcessingEvent::~SnsSimulationHostProcessingEvent() {
    if (((SnsHost*)this->host)->host_proc_sns_sim_event == this) {
        ((SnsHost*)this->host)->host_proc_sns_sim_event = NULL;
    }
}

void SnsSimulationHostProcessingEvent::process_event() {
    ((SnsHost*)this->host)->host_proc_sns_sim_event = NULL;
    ((SnsHost*)this->host)->move_sim_to_packets();
}

SnsSendPacketProcessingEvent::SnsSendPacketProcessingEvent(double time, SnsHost *h) : Event(HOST_PROCESSING, time) {
    this->host = h;
    this->is_timeout = false;
}

SnsSendPacketProcessingEvent::~SnsSendPacketProcessingEvent() {
    if (((SnsHost*)this->host)->host_proc_sns_send_packet_event == this) {
        ((SnsHost*)this->host)->host_proc_sns_send_packet_event = NULL;
    }
}

void SnsSendPacketProcessingEvent::process_event() {
    ((SnsHost*)this->host)->host_proc_sns_send_packet_event = NULL;
    ((SnsHost*)this->host)->move_packets_to_queue();
}


SnsAddToPktToBeSentEvt::SnsAddToPktToBeSentEvt(double time, SnsHost *h, SnsPacket *pkt) : Event(HOST_PROCESSING, time) {
    this->host = h;
    this->pkt = pkt;
    this->is_timeout = false;
}

SnsAddToPktToBeSentEvt::~SnsAddToPktToBeSentEvt() {
    if (((SnsHost*)this->host)->host_proc_sns_add_to_pkt_to_be_sent_event == this) {
        ((SnsHost*)this->host)->host_proc_sns_add_to_pkt_to_be_sent_event = NULL;
    }
}

void SnsAddToPktToBeSentEvt::process_event() {
    ((SnsHost*)this->host)->host_proc_sns_add_to_pkt_to_be_sent_event = NULL;
    ((SnsHost*)this->host)->packets_to_be_sent.push_back(this->pkt);
}
