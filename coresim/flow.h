...
class Flow {
    public:
        ...
        virtual ~Flow(); // Virtual destructor
        ...
        //virtual void send_ack(uint32_t seq, std::vector<uint32_t> sack_list, uint32_t seq_for_sns);
        ...
        void log_drop_reorder(int is_drop, int is_reorder, int total_pkts);
        void log_queuing();
        void log_utilization_include_sim(int pkt_size);
        ...
        // For SNS
        std::multiset<uint32_t> sim_ids_sent;
        std::multiset<uint32_t> sim_ack_ids_sent;
        std::multiset<uint32_t> data_ids_sent;
        std::multiset<uint32_t> data_ids_recived;
        int sns_sim_pkt_drop;
        float flow_rtt;

        uint32_t last_id_received;
        uint32_t num_of_reorders;
