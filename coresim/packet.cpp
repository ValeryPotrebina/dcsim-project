...
Packet::Packet(
    ...
    this->sns_type = 0;
    this->saved_hash_port = -1;
    this->saved_hash_port_l2 = -1;
    this->sim_enter_q1 = -1;
    this->sim_enter_q2 = -1;
    this->data_enter_q2 = -1;
}
...
Ack::Ack(Flow *flow, uint32_t seq_no_acked, std::vector<uint32_t> sack_list, uint32_t size, Host* src, Host *dst, uint32_t seq_for_sns) : Packet(0, flow, seq_no_acked, 0, size, src, dst) {
    this->type = ACK_PACKET;
    this->sack_list = sack_list;
}

RTSCTS::RTSCTS(bool type, double sending_time, Flow *f, 
    uint32_t size, Host *src, Host *dst) : Packet(sending_time, f, 0, 0, f->hdr_size, src, dst) {
    if (type) {
...
SnsPacket::SnsPacket(int type, double sending_time, Flow *f, uint32_t seq, uint32_t priority,
                        uint32_t size, Host *src, 
                        Host *dst, int saved_hash_port, int msg_id) : Packet(sending_time, f, seq, priority, size, src, dst) {
    this->sns_type = type;
    if (type == SNS_SIM_PACKET) {        
        //this->size /= SNS_SIMULATION_FRAQ_SIZE;
        this->size = params.hdr_size; //Minimum packet size     

    }
    if (type == SNS_SIM_ACK_PACKET) {        
  
        this->size = params.hdr_size; //Minimum packet size        
    }    
    
    this->saved_hash_port = saved_hash_port;
    this->sns_msg_id = msg_id;    
    if (type == SNS_SIM_ACK_PACKET)
        this->type = ACK_PACKET;
}
