...
FatTreeTopology::FatTreeTopology(
    ...
    for (uint32_t i = 0; i < num_hosts; i++) {
        ...
        //  std::cout << "Linking Host " << i << " to Edge " << edge_switches[2 * i / k]->id  << " with queue " <<  hosts[i]->queue->id << " " <<  hosts[i]->queue->unique_id << "\n";
    }
    ...
    for (uint32_t i = 0; i < num_edge_switches; i++) {
    	...
    	for (uint32_t j = 0; j < k / 2; j++) {
            ...
            //  std::cout << "Linking Edge  " << edge_switches[i]->id << " to Agg " << agg_switches[ pod * k / 2 + j]->id  << " with queue " << q->id << " " << q->unique_id << "\n";
        }
    ...
    for (uint32_t i = 0; i < num_agg_switches; i++) {
        ...
        for (uint32_t j = 0; j < k / 2; j++) { // TODO make generic
            ...
            //  std::cout << "Linking Agg " << agg_switches[i]->id << " to Edge" << edge_switches[pod * k / 2 + j]->id  << " with queue " << q->id << " " << q->unique_id << "\n";
        }
        // Queues to Core
        for (uint32_t j = 0; j < k / 2; j++) {
            Queue *q = agg_switches[i]->queues[j + k / 2];
            // Map to available core switches using modulo
            uint32_t core_idx = (j + base) % num_core_switches;
            q->set_src_dst(agg_switches[i], core_switches[core_idx]);
            //  std::cout << "Linking Agg " << agg_switches[i]->id << " to Core" << core_switches[core_idx]->id  << " with queue " << q->id << " " << q->unique_id << "\n";
        }
    }
    ...
    for (uint32_t i = 0; i < num_core_switches; i++) {
        ...
        for (uint32_t j = 0; j < k; j++) {
            ...
            uint32_t agg_idx = (k / 2 * j + base) % num_agg_switches;
            q->set_src_dst(core_switches[i], agg_switches[agg_idx]);
            //  std::cout << "Linking Core " << core_switches[i]->id << " to Agg" << agg_switches[agg_idx]->id  << " with queue " << q->id << " " << q->unique_id << "\n";
        }
    }
    // Set arbiter
...
Queue *FatTreeTopology::get_next_hop_inner(Packet *p, Queue *q) {
    if (q->dst->type == HOST) {
...
Queue *FatTreeTopology::get_next_hop(Packet *p, Queue *q) {
    Queue* next_hop = get_next_hop_inner(p, q);
    if (!next_hop){
        return NULL; // Packet Arrival
    }
    // while (next_hop->unique_id <= 638 && next_hop->unique_id >= 390 
    //     && (((next_hop->unique_id - 390) % 8 == 0) || ((next_hop->unique_id - 390) % 8 == 1))) {
    //     next_hop = get_next_hop_inner(p, q);
    //     // std::cout << "SnsFatTreeTopology::get_next_hop next_hop->unique_id " << next_hop->unique_id << "((SnsSwitch *)q->src)->switch_type" << ((Switch *)q->src)->switch_type << "\n";
    // }
    return next_hop;
